module Oak
  class Parser
     ...
     Treetop.load(File.expand_path(File.join(File.dirname(__FILE__), 'oakparser.treetop')))
     @@parser = OakParser.new
     ...
     def self.parse(data)
       ...
       if(tree.nil?)
         ...
         raise Exception, "(AST nil) Parse error at offset: #{@@parser.index}"
       end
